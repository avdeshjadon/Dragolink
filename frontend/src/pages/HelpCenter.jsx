...
export default function HelpCenter() {
  ...
  if (!data || !data.hero) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-bg-light text-text-secondary">
        Help Center data not available.
      </div>
    );
  }
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            ...
            <div className="mt-10 flex flex-wrap justify-center gap-3">
              <span className="text-text-secondary text-sm font-medium mr-2 flex items-center">
                Trending:
              </span>
              {trending.map((topic, i) => (
                <button
                  key={i}
                  className="px-4 py-1.5 rounded-full bg-surface-light border border-border-light text-sm text-text-secondary hover:bg-black/5 hover:text-brand-dark transition-colors shadow-sm"
                >
                  {topic}
                </button>
              ))}
