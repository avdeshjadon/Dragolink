...
export default function BlogPost() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-bg-light flex items-center justify-center font-sans">
        Loading post...
      </div>
    );
  }
    ...
    <div className="bg-bg-light min-h-screen font-sans pb-24 text-brand-dark">
      ...
      <div className="bg-[#F9F7F1] pt-32 pb-16 px-4 sm:px-6 lg:px-8 border-b border-border-light">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            <Link
              to="/blog"
              className="inline-flex items-center text-text-secondary hover:text-brand transition-colors mb-10 text-sm font-medium"
            >
                ...
                JSON.parse(post.content).map((section, idx) => (
                  <div key={idx} className="mb-12">
                    {section.heading && (
                      <h2
                        id={`section-${idx}`}
                        className="text-3xl font-bold text-brand-dark mb-6 tracking-tight"
                      >
                        {section.heading}
                      </h2>
                    )}
                          ...
                          <thead>
                            <tr className="bg-surface-light border-b border-border-light">
                              {section.table.headers.map((header, cIdx) => (
                                <th
                                  key={cIdx}
                                  className="py-4 px-6 text-sm font-bold text-brand-dark uppercase tracking-wider"
                                >
                                  {header}
                                </th>
                              ))}
                            ...
                            {section.table.rows.map((row, rIdx) => (
                              ...
                              >
                                {row.map((cell, cIdx) => (
                                  <td
                                    key={cIdx}
                                    className="py-4 px-6 text-base text-text-secondary"
                                  >
                                    {cell}
                                  </td>
                                ))}
          ...
          <aside className="lg:col-span-4 hidden lg:block">
            <div className="sticky top-32 space-y-6">
                    ...
                    JSON.parse(post.content).map((section, idx) =>
                      section.heading ? (
                        <li key={idx}>
                          <a
                            href={`#section-${idx}`}
                            className="text-text-secondary hover:text-brand font-medium transition-colors line-clamp-2"
                          >
                            {section.heading}
                          </a>
                        </li>
              ...
              <div className="bg-[#F9F7F1] rounded-[2rem] p-8 flex items-center justify-between">
                <span className="text-sm font-bold tracking-widest text-brand-dark uppercase">
                  Share:
                </span>
                <div className="flex gap-2">
                  <Button
                    variant="ghost"
                    className="w-10 h-10 p-0 rounded-full bg-white hover:bg-brand/10 text-brand-dark shadow-sm"
                  >
      ...
      <div className="px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto mt-24 pt-12 border-t border-border-light text-center">
        ...
        <p className="text-text-secondary mb-8">
          Share it with your network or read more from the Dragolink team.
        </p>
        <div className="flex justify-center gap-4">
          <Link to="/blog">
            <Button
              variant="outline"
              className="border-border-light hover:bg-surface-light text-brand-dark"
            >
              More Articles
            </Button>
          </Link>
