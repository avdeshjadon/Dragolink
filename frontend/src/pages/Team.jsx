          ...
          <div className="grid grid-cols-1 gap-2">
            {loading ? (
              ...
              members.filter(m => m.status === 'ACTIVE').map(member => (
                <div key={member.id} className="bg-surface-container border border-outline-variant/10 rounded-lg p-4 flex items-center justify-between hover:border-primary/30 hover:bg-surface-container-high transition-all duration-200 group">
                  <div className="flex items-center gap-4">
                    {member.profileImage ? (
                      <img className="w-10 h-10 rounded-full object-cover border border-outline-variant/20" alt={member.name} src={member.profileImage}/>
                    ) : (
                      <div className="w-10 h-10 rounded-full bg-secondary-container text-on-secondary-container flex items-center justify-center font-bold text-sm border border-secondary/20">
                        {(member.name ? member.name.substring(0, 2) : 'U').toUpperCase()}
                      </div>
                    )}
                    <div>
                      <p className="text-label-md font-label-md text-on-background">{member.name || 'Unknown User'}</p>
                      <p className="text-label-sm font-label-sm text-on-surface-variant font-code-sm">{member.email}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-6">
                    <select 
                      ...
                      className={`inline-flex items-center px-2 py-1 rounded-full text-label-sm font-label-sm outline-none border border-transparent hover:border-outline-variant/30 ${
                        member.role === 'ADMIN' ? 'bg-secondary-container text-on-secondary-container shadow-sm' : 'bg-surface-container-highest text-on-surface'
                      }`}
                    >
          ...
          <div className="grid grid-cols-1 gap-2">
            {members.filter(m => m.status === 'INVITED').length > 0 ? (
              members.filter(m => m.status === 'INVITED').map(member => (
                <div key={member.id} className="bg-surface-container-lowest border border-outline-variant/20 border-dashed rounded-lg p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-primary/50 transition-all duration-200">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-label-sm font-label-sm text-primary font-code-sm">{member.email}</p>
                    </div>
                  </div>
              ...
              <div className="p-8 text-center text-on-surface-variant">No pending invitations.</div>
            )}
          </div>
        </section>
      </div>
