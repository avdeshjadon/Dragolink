...
export default function PublicIntegrations() {
  ...
  if (!data || !data.hero) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-bg-light text-text-secondary">
        Integrations data not available.
      </div>
    );
  }
    ...
    <div className="bg-bg-light min-h-screen font-sans">
      ...
      <section className="py-24 bg-surface-light border-y border-border-light text-center px-4">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-lg text-text-secondary mb-8">{cta.description}</p>
          <a
            href={
              cta.buttonEmail.includes("@")
                ? `mailto:${cta.buttonEmail}`
                : cta.buttonEmail
            }
          >
