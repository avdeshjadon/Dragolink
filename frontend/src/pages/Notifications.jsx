...
export default function Notifications() {
  const [notifications, setNotifications] = useState([
    ...
    },
    {
      ...
      message: 'You can now view detailed analytics for QR codes in your dashboard.',
      time: '1 hour ago',
      read: false
    },
    {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-on-surface tracking-tight">Notifications</h1>
          <p className="text-on-surface-variant">Stay updated with your account activity.</p>
        </div>
        <button 
        ...
        <AnimatePresence>
          {notifications.length === 0 ? (
            ...
            >
              ...
              <h2 className="text-xl font-bold text-on-surface mb-2">No new notifications</h2>
              <p className="text-on-surface-variant max-w-md mx-auto">You're all caught up! Check back later for new updates.</p>
            </motion.div>
          ) : (
            notifications.map((notification) => (
              ...
              >
                ...
                <div className="flex-1 pr-8">
                  <h3 className={`font-bold text-lg mb-1 ${notification.read ? 'text-on-surface-variant' : 'text-on-surface'}`}>
                    {notification.title}
                  </h3>
