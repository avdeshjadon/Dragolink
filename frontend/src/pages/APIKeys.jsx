...
export default function APIKeys() {
  ...
  return (
    <div className="flex-1 p-6 md:p-10 max-w-7xl mx-auto w-full flex flex-col">
      ...
      {loading ? (
        <div className="flex justify-center items-center py-20">Loading API keys...</div>
      ) : keys.length > 0 ? (
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="border-b border-outline-variant/10 bg-surface-container-lowest">
                <th className="p-4 text-label-sm font-label-sm text-on-surface-variant uppercase tracking-wider font-medium">Name</th>
                <th className="p-4 text-label-sm font-label-sm text-on-surface-variant uppercase tracking-wider font-medium">Key Prefix</th>
                <th className="p-4 text-label-sm font-label-sm text-on-surface-variant uppercase tracking-wider font-medium">Created</th>
                <th className="p-4 text-label-sm font-label-sm text-on-surface-variant uppercase tracking-wider font-medium">Last Used</th>
                <th className="p-4 text-right"></th>
              </tr>
            </thead>
            <tbody className="text-body-md font-body-md">
              {keys.map(key => (
                <tr key={key.id} className="border-b border-outline-variant/5 hover:bg-surface-container-high transition-colors">
                  <td className="p-4 text-on-surface font-medium">{key.name}</td>
                  <td className="p-4"><span className="font-code-sm text-code-sm text-on-surface-variant bg-surface-variant px-2 py-1 rounded">{key.prefix}••••••••••••</span></td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-surface border border-outline-variant/20 rounded-2xl w-full max-w-md shadow-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-outline-variant/10 flex justify-between items-center">
              <h3 className="text-title-lg font-title-lg text-on-surface">Generate API Key</h3>
              <button onClick={() => { setIsModalOpen(false); setGeneratedKey(null); }} className="text-on-surface-variant hover:text-on-surface p-1 rounded-full hover:bg-surface-variant transition-colors">
              ...
              {generatedKey ? (
                <div className="space-y-4">
                  <div className="p-4 bg-error-container/20 border border-error/20 rounded-lg flex gap-3 text-on-surface">
                    ...
                    <p className="text-sm">Please copy this key and store it securely. For your protection, you will not be able to see it again.</p>
                  </div>
                  <div>
                    <label className="block text-label-sm font-label-md text-on-surface-variant mb-1">Your API Key</label>
                    <div className="flex">
                      <input 
                        type="text" 
                        readOnly 
