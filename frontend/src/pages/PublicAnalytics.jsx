...
export default function PublicAnalytics() {
  ...
  useEffect(() => {
    api
      .get("/public/pages/public-analytics")
      .then((res) => {
  ...
  if (!data || !data.hero) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-bg-light text-text-secondary">
        Analytics Engine data not available.
      </div>
    );
  }
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link to="/register">
                <Button
                  size="lg"
                  className="h-14 px-8 text-lg w-full sm:w-auto shadow-lg shadow-brand/20"
                >
                  Start Tracking Free
                </Button>
              </Link>
              <Link to="/pricing">
                <Button
                  size="lg"
                  variant="outline"
                  className="h-14 px-8 text-lg w-full sm:w-auto bg-surface-light border-2"
                >
                  View Pricing
                </Button>
              </Link>
          ...
          >
            ...
            <div className="relative bg-surface-light border border-border-light rounded-3xl p-8 shadow-2xl">
              ...
              <div className="flex items-end gap-1.5 h-24 mb-6">
                {[40, 65, 45, 80, 55, 90, 70, 95, 60, 100, 75, 88].map(
                  (h, i) => (
                    <motion.div
                      key={i}
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-brand-dark mb-6">
            Every insight you need, in one place
          </h2>
          <p className="text-lg text-text-secondary">
            Built for marketers, growth teams, and enterprises that treat data
        ...
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
          ...
          <div>
            ...
            <p className="text-text-secondary">
              All analytics are collected without third-party cookies. We are
              GDPR, CCPA, and PECR compliant out of the box. Your data stays
              yours.
            </p>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto bg-brand-dark rounded-[2.5rem] p-12 md:p-20 text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-lg text-white/80 mb-10 max-w-2xl mx-auto">
              Create your first tracked link free. No credit card required. Full
              analytics dashboard available from day one.
            </p>
            <Link to="/register">
              <Button
                size="lg"
                className="h-14 px-10 text-lg bg-brand-emerald hover:bg-brand text-brand-dark hover:text-white transition-all shadow-xl"
              >
