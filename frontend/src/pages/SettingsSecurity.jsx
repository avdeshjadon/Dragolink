      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="bg-surface-container-low border border-outline-variant/10 shadow-sm rounded-xl p-6 hover:shadow-md transition-all duration-300">
            ...
            {passwordMessage && (
              <div className={`mb-4 p-3 rounded text-sm ${passwordMessage.includes('success') ? 'bg-primary/20 text-primary' : 'bg-error/20 text-error'}`}>
                {passwordMessage}
              </div>
            )}
            <form className="flex flex-col gap-4" onSubmit={handlePasswordChange}>
              <div className="flex flex-col gap-1">
                ...
                <input 
                  ...
                  required
                  className="bg-surface-dim border border-outline-variant/30 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all rounded-lg px-4 py-2 text-body-md font-body-md font-code-sm" type="password"
                />
              </div>
              <div className="flex flex-col gap-1">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-outline-variant/10 bg-surface-container-lowest/50">
                    <th className="py-3 px-6 text-label-sm font-label-sm text-on-surface-variant uppercase tracking-wider font-medium">Event</th>
                    ...
                    <th className="py-3 px-6 text-label-sm font-label-sm text-on-surface-variant uppercase tracking-wider font-medium">Device & Location</th>
                    <th className="py-3 px-6 text-right text-label-sm font-label-sm text-on-surface-variant uppercase tracking-wider font-medium">Time</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-outline-variant/5">
                  {logsLoading ? (
                    <tr>
                      <td colSpan="4" className="py-8 text-center text-on-surface-variant">Loading security logs...</td>
                    </tr>
                  ) : logs.length > 0 ? (
                    logs.map(log => (
                      <tr key={log.id} className="hover:bg-surface-container-high/30 transition-colors group">
                        <td className="py-4 px-6">
                          <div className="text-body-md font-body-md font-medium text-on-surface">{log.eventType}</div>
                        </td>
                        <td className="py-4 px-6">
                          <span className="text-code-sm font-code-sm text-on-surface-variant">{log.ipAddress || 'Unknown'}</span>
                        </td>
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-4">
                            <div>
                              <div className="text-body-md font-body-md font-medium text-on-surface">{log.device || 'Unknown Device'}</div>
                              <div className="text-label-sm font-label-sm text-on-surface-variant mt-0.5">{log.location || 'Unknown Location'}</div>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-6 text-right">
                          <span className="text-body-md font-body-md text-on-surface-variant">{new Date(log.createdAt).toLocaleString()}</span>
                        </td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan="4" className="py-8 text-center text-on-surface-variant">No security logs found.</td>
                    </tr>
                  )}
                </tbody>
              </table>
