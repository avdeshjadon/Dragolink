...
import { TrendingUp, Quote, ArrowRight, Play, CheckCircle2 } from 'lucide-react';
...
export default function CaseStudies() {
  ...
  useEffect(() => {
    api.get('/public/pages/case-studies')
      .then(res => {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center bg-bg-light text-text-secondary">Loading Case Studies...</div>;
  }

  if (!data || !data.hero) {
    return <div className="min-h-screen flex items-center justify-center bg-bg-light text-text-secondary">Case Studies data not available.</div>;
  }
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto p-4 bg-surface-light backdrop-blur-xl border border-border-light rounded-3xl shadow-xl">
              {hero.stats.map((stat, i) => (
                <div key={i} className="text-center p-4">
                  <div className="text-3xl lg:text-4xl font-extrabold text-brand-dark mb-1">{stat.val}</div>
          ...
          >
            {filteredCases.length > 0 ? (
              filteredCases.map((study, idx) => (
                ...
                >
                  ...
                  <div className="w-full lg:w-1/2 relative group">
                    <div className={`absolute inset-0 bg-gradient-to-br ${study.color} rounded-[3rem] blur-2xl opacity-80 group-hover:opacity-100 transition-opacity duration-1000`} />
                    <div className="relative aspect-square lg:aspect-auto lg:h-[600px] w-full bg-surface-light border border-border-light rounded-[3rem] overflow-hidden p-12 flex flex-col justify-between shadow-2xl">
                      ...
                      <div className="flex justify-between items-center relative z-10">
                        <div className={`text-sm font-bold tracking-widest uppercase ${study.accent}`}>{study.industry}</div>
                        <div className={`w-12 h-12 rounded-full ${study.iconColor} shadow-lg flex items-center justify-center backdrop-blur-sm`}>
                           <TrendingUp className="w-5 h-5" />
                        </div>
                      </div>

                      <div className="relative z-10 text-center flex-1 flex items-center justify-center">
                        <div className={`text-5xl font-black ${study.accent} opacity-80`}>{study.company}</div>
                      </div>
                      
                      <div className="relative z-10 bg-white/80 backdrop-blur-xl border border-border-light rounded-3xl p-8 shadow-md">
                        <Quote className={`w-8 h-8 ${study.accent} mb-4 opacity-70`} />
                        <p className="text-lg text-brand-dark font-medium leading-relaxed italic mb-4">
                          "{study.quote}"
                        </p>
                        <p className="text-sm text-text-secondary font-bold tracking-wide uppercase">— {study.author}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="w-full lg:w-1/2 flex flex-col justify-center">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-12">
                      {study.metrics.map((m, i) => (
                        <div key={i} className="border-l-4 border-border-light pl-6">
                          <div className={`text-3xl font-extrabold ${study.accent} mb-2`}>{m.value}</div>
                          <div className="text-sm text-text-secondary font-bold">{m.label}</div>
                        </div>
                      ))}
                    </div>
                    
                    <div className="flex flex-col sm:flex-row gap-4">
                      <Button size="lg" className="h-14 px-8 bg-brand-dark text-white hover:bg-brand-dark/90 shadow-lg">
                        Read Full Story
                      </Button>
                      <Button size="lg" variant="outline" className="h-14 px-8 text-brand-dark bg-surface-light border-border-light hover:bg-black/5">
              ...
              <div className="text-center py-32">
                <p className="text-2xl text-text-secondary font-medium">No case studies found for this industry.</p>
                <p className="text-text-secondary mt-2">Check back later or browse other categories.</p>
              </div>
            )}
          </motion.div>
        </AnimatePresence>
      ...
      <section className="px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto mb-24">
        <div className="bg-gradient-to-r from-brand-emerald to-brand rounded-[3rem] p-1 border border-brand/20 shadow-2xl relative overflow-hidden group">
          ...
          <div className="bg-brand-dark rounded-[2.8rem] p-12 md:p-20 text-center relative z-10">
            <h2 className="text-4xl md:text-5xl font-extrabold text-white mb-6">{cta.title}</h2>
            <p className="text-xl text-white/70 mb-10 max-w-2xl mx-auto">{cta.description}</p>
            <Button size="lg" className="h-16 px-10 text-lg bg-brand-emerald text-brand-dark hover:bg-brand shadow-xl shadow-brand-emerald/20 border-none font-bold">
            ...
            <ul className="mt-8 flex flex-wrap justify-center gap-6 text-sm font-bold text-white/70">
              <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-brand-emerald" /> Personalized Demo</li>
              <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-brand-emerald" /> ROI Calculation</li>
              <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-brand-emerald" /> Custom Pricing</li>
            </ul>
          </div>
        </div>
      </section>
