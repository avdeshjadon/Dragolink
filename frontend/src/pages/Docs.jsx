...
export default function Docs() {
  ...
  useEffect(() => {
    if (!data) return;
    const handleScroll = () => {
      const sectionElements = data.sections.map((s) =>
    ...
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center bg-bg-light text-text-secondary gap-4">
        ...
        <button
          ...
          className="px-5 py-2 mt-2 bg-brand text-white rounded-lg font-bold shadow-sm hover:bg-brand-dark transition-colors cursor-pointer"
        >
          Try Again
        </button>
      </div>
    ...
    <div className="bg-bg-light min-h-screen font-sans text-brand-dark pt-24 pb-24 flex justify-center">
      <div className="max-w-[1400px] w-full px-4 sm:px-6 lg:px-8 flex items-start gap-12">
        ...
        <aside className="w-64 hidden lg:block sticky top-32 shrink-0">
          <h2 className="text-sm font-bold tracking-wider text-text-secondary uppercase mb-6">
            Documentation
          </h2>
                ...
                section.endpoints.map((ep, i) => (
                  <div key={i} className="mb-12">
                    ...
                    <div className="flex items-center gap-3 mb-6 bg-surface-light p-3 rounded-lg border border-border-light w-fit">
                      <span
                        className={`font-bold px-2 py-1 rounded text-xs ${ep.method === "GET" ? "bg-blue-500/10 text-blue-600" : "bg-green-500/10 text-green-600"}`}
                      >
                        {ep.method}
                      </span>
                        ...
                        <div className="border border-border-light rounded-xl overflow-hidden mb-8">
                          <table className="w-full text-sm text-left">
                            <thead className="bg-surface-light text-text-secondary">
                              <tr>
                                <th className="px-4 py-3 font-semibold border-b border-border-light">
                                  Property
                                </th>
                                <th className="px-4 py-3 font-semibold border-b border-border-light">
                                  Type
                              ...
                              {ep.parameters.map((param, j) => (
                                <tr key={j}>
                                  <td className="px-4 py-3 align-top">
                                    <span className="font-mono font-bold text-brand-dark block mb-1">
                                      {param.name}
                                    </span>
                                    {param.required && (
                                  ...
                                  <td className="px-4 py-3 font-mono text-text-secondary align-top">
                                    {param.type}
                                  </td>
                  ...
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-border-light">
                      {section.errorsList.map((err, i) => (
                        <tr key={i}>
                          <td className="px-4 py-3 font-mono font-bold text-brand-dark whitespace-nowrap">
                            {err.code}
                          </td>
