...
export default function MyLinks() {
  ...
  const handleDeleteLog = async (logId) => {
    if (!clickLogModalLink) return;
    try {
      setIsDeletingLog(true);
      await api.delete(`/analytics/links/${clickLogModalLink.id}/logs/${logId}`);
      setClickLogs(prev => prev.filter(log => log.id !== logId));
  ...
  const handleClearAllLogs = async () => {
    ...
    try {
      setIsDeletingLog(true);
      await api.delete(`/analytics/links/${clickLogModalLink.id}/logs`);
      setClickLogs([]);
  ...
  const downloadLogsCSV = () => {
    ...
    const headers = ['Time', 'IP Address', 'ISP', 'Location', 'Device', 'OS', 'Browser', 'Referrer', 'Referrer Channel', 'Is Bot', 'Is QR', 'UTM Source', 'UTM Medium', 'UTM Campaign', 'UTM Term', 'UTM Content'];
            ...
            <div className="flex flex-col gap-4">
              <AnimatePresence>
                {clickLogs.map((log, idx) => {
                  ...
                  const isDeleting = isDeletingLog && deleteLogModalId === (log.id || idx);
                  ...
                  return (
                    <motion.div 
                      ...
                      className="bg-surface/50 border border-outline-variant/30 rounded-2xl p-5 hover:bg-surface-container-lowest hover:shadow-md transition-all duration-300"
                    >
                      {/* Top Section: Time, IP, and ISP */}
                      <div 
                        className={`flex flex-col lg:flex-row lg:items-center justify-between gap-4 cursor-pointer select-none ${isExpanded ? 'mb-4 border-b border-outline-variant/10 pb-4' : ''}`}
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="flex flex-wrap items-center gap-2 mb-1">
                              ...
                              {log.isBot && <span className="text-[10px] font-bold px-1.5 py-0.5 bg-error/10 text-error rounded-sm tracking-wider uppercase">Bot</span>}
                              {log.qrScan && <span className="text-[10px] font-bold px-1.5 py-0.5 bg-primary/10 text-primary rounded-sm tracking-wider uppercase">QR Scan</span>}
                            </div>
                        ...
                        {/* Right Area: Device Pills & Controls */}
                        <div className="flex items-center justify-between lg:justify-end gap-4">
                          ...
                          <div className="flex items-center gap-2">
                            {log.id && (
                              <button 
                                onClick={(e) => { e.stopPropagation(); setDeleteLogModalId(log.id); }}
                                className="p-2 text-on-surface-variant hover:text-error hover:bg-error/10 rounded-full transition-colors flex items-center justify-center cursor-pointer disabled:opacity-50"
                                disabled={isDeleting}
                                title="Delete Log"
                              >
                                {isDeleting ? (
                                  <span className="w-5 h-5 border-2 border-error border-t-transparent rounded-full animate-spin block"></span>
                                ) : (
                                  <span className="material-symbols-outlined text-[20px]">delete</span>
                                )}
                              </button>
                            )}
                            <button className="p-2 text-on-surface-variant rounded-full hover:bg-surface-container transition-colors flex items-center justify-center">
                              <span className={`material-symbols-outlined text-[24px] transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`}>
                                expand_more
                              </span>
                            </button>
                          </div>
                          ...
                          >
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                              ...
                              <div className="flex flex-col gap-3">
                                <div className="flex items-start gap-3 p-3 rounded-xl bg-surface-container-lowest border border-outline-variant/10">
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <div className="flex items-center justify-between gap-2 mb-1">
                                      <p className="text-label-md font-semibold text-on-surface">Referrer</p>
                                      {log.referrerChannel && log.referrerChannel !== 'Direct' && (
                                        <span className="text-[10px] font-bold px-1.5 py-0.5 bg-surface-container-high text-on-surface-variant rounded-sm tracking-wider uppercase">{log.referrerChannel}</span>
                                      )}
                                    </div>
                                ...
                                {(log.utmSource || log.utmMedium || log.utmCampaign || log.utmTerm || log.utmContent) && (
                                  <div className="flex items-start gap-3 p-3 rounded-xl bg-surface-container-lowest border border-outline-variant/10">
                                    <div className="p-2 bg-secondary/10 rounded-lg text-secondary mt-0.5">
                                      <span className="material-symbols-outlined text-[20px]">campaign</span>
                                    </div>
                                    <div className="min-w-0 flex-1">
                                      <p className="text-label-md font-semibold text-on-surface mb-2">UTM Parameters</p>
                                      <div className="flex flex-col gap-1 text-body-sm">
                                        {log.utmSource && <div className="flex"><span className="text-on-surface-variant w-20 shrink-0">Source:</span><span className="text-on-surface font-medium truncate">{log.utmSource}</span></div>}
                                        {log.utmMedium && <div className="flex"><span className="text-on-surface-variant w-20 shrink-0">Medium:</span><span className="text-on-surface font-medium truncate">{log.utmMedium}</span></div>}
                                        {log.utmCampaign && <div className="flex"><span className="text-on-surface-variant w-20 shrink-0">Campaign:</span><span className="text-on-surface font-medium truncate">{log.utmCampaign}</span></div>}
                                        {log.utmTerm && <div className="flex"><span className="text-on-surface-variant w-20 shrink-0">Term:</span><span className="text-on-surface font-medium truncate">{log.utmTerm}</span></div>}
                                        {log.utmContent && <div className="flex"><span className="text-on-surface-variant w-20 shrink-0">Content:</span><span className="text-on-surface font-medium truncate">{log.utmContent}</span></div>}
                                      </div>
                                    </div>
                                  </div>
                                )}
