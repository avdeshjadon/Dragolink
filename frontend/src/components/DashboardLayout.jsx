...
export default function DashboardLayout() {
  ...
  useEffect(() => {
    api.get('/public/navigation?position=DASHBOARD_SIDEBAR')
      .then(res => setNavigation(res.data.filter(item => item.label !== 'QR Codes' && item.label !== 'Admin')))
  ...
  const SidebarContent = () => {
    ...
    <nav className="flex h-full flex-col bg-surface-container-low w-64">
      <div className="flex-1 overflow-y-auto p-4 flex flex-col">
        ...
        <div className="flex-1 space-y-2">
          {navigation.map(item => {
            ...
            if (isActive) {
              return (
                <Link key={item.id} to={item.url} className="flex items-center gap-4 bg-secondary-container text-on-secondary-container rounded-lg px-4 py-2 shadow-sm transition-all duration-300 ease-out hover:scale-[1.02] active:scale-[0.98]">
            ...
            return (
              <Link key={item.id} to={item.url} className="flex items-center gap-4 text-on-surface-variant hover:bg-surface-container-high rounded-lg px-4 py-2 transition-all duration-300 ease-out hover:scale-[1.02] active:scale-[0.98]">
                <span className="material-symbols-outlined">{item.badgeText}</span>
                <span className="font-label-md text-label-md">{item.label}</span>
              </Link>
            );
          ...
          <Link to="/notifications" className={`flex items-center gap-4 rounded-lg px-4 py-2 transition-all duration-300 ease-out hover:scale-[1.02] active:scale-[0.98] ${location.pathname.startsWith('/notifications') ? 'bg-secondary-container text-on-secondary-container shadow-sm' : 'text-on-surface-variant hover:bg-surface-container-high'}`}>
            <span className="material-symbols-outlined">notifications</span>
            <span className="font-label-md text-label-md">Notification</span>
          </Link>
          <Link to="/settings/profile" className={`flex items-center gap-4 rounded-lg px-4 py-2 transition-all duration-300 ease-out hover:scale-[1.02] active:scale-[0.98] ${location.pathname.startsWith('/settings') ? 'bg-secondary-container text-on-secondary-container shadow-sm' : 'text-on-surface-variant hover:bg-surface-container-high'}`}>
      ...
      <div className="p-4 space-y-2 border-t border-outline-variant/10 bg-surface-container-low z-10">
        ...
        <Link to="/help" className="flex items-center gap-4 text-on-surface-variant hover:bg-surface-container-high rounded-lg px-4 py-2 transition-all duration-300 ease-out hover:scale-[1.02] active:scale-[0.98]">
          <span className="material-symbols-outlined">help</span>
          <span className="font-label-md text-label-md">Help Center</span>
        </Link>
        <button onClick={logout} className="w-full flex items-center gap-4 text-on-surface-variant hover:bg-surface-container-high rounded-lg px-4 py-2 transition-all duration-300 ease-out hover:scale-[1.02] active:scale-[0.98]">
