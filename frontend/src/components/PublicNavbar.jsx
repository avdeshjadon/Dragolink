...
export default function PublicNavbar() {
  ...
  useEffect(() => {
    api
      .get("/public/navigation?position=PUBLIC_HEADER")
      .then((res) => {
        setLinks(res.data);
      })
      .catch((err) => {
        ...
        <div className="flex items-center">
          <Link to="/" className="flex items-center gap-2">
            <img
              ...
              className="h-8 w-8"
            />
            <span className="text-xl font-bold text-[#16803C] tracking-tight">
              Dragolink
            </span>
          </Link>
        </div>
        ...
        <div className="flex items-center justify-end gap-4">
          {user ? (
            <Link to="/dashboard">
              <Button
                size="sm"
                className="flex items-center gap-2 cursor-pointer shadow-md bg-brand text-white hover:bg-brand-dark transition-colors"
              >
